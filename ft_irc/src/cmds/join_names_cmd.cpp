...
void Server::join_names_command( Request request )
{
	...
	while (!channelNames.empty ())
	{
		...
		if (request.get_cmd () == "JOIN" && it->second.check_is_member (request.get_user ()))
		...
		else if (request.get_cmd () == "JOIN")
		{
			...
			if (it->second.check_has_mode('l') && it->second.getLimit() != -1 && static_cast<int>(it->second.getMembers().size()) >= it->second.getLimit())
			{

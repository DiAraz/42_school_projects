...
/*   Updated: 2024/09/20 09:58:30 by daraz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	else
	{
		...
		else
		{
			...
			for (it = targets.begin(); it != targets.end(); ++it)
			{
				...
				// ERR_NOSUCHNICK
				if (get_user_by_nick(*it) == _user_map.end() && _channels.find(*it) == _channels.end())
				{
			...
			if (response.empty())
			{
				...
				{
					// send to user
					if ((recipient = get_user_by_nick(*it)) != _user_map.end())

...
/*   Updated: 2024/09/30 11:12:51 by daraz            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::ft_join( Request request )
{
    ...
    while (!channelNames.empty()) 
    {
        ...
        if (request.get_cmd() == "JOIN") 
        {
            ...
            if (it->second.check_has_mode('i') && !request.get_user()->is_invited()) 
            {
                send_message(request, RES_ERR_INVITEONLYCHAN);
                continue;
            }
            ...
            Bot welcome_bot(-1, "BOT");
            std::string welcome_message = "Welcome to " + it->second.getName() + " channel, " + request.get_user()->get_nickname() + "!";
            send_message(welcome_bot.get_prefix() + " PRIVMSG " + it->second.getName() + " :" + welcome_message, request.get_user()->get_fd());
        }
